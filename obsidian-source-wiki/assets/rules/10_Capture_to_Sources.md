...
- `00_Capture/External/`  ← 外部内容（抖音、小红书等链接）
- `00_Capture/Personal/`  ← 个人随记、碎点子

文件可以没有名字。agent 读内容，不依赖文件名。
...
Get-ChildItem "vault\00_Capture\Personal" -Recurse -File | Sort-Object LastWriteTime -Descending | Select-Object -First 20 FullName, LastWriteTime
```
...
## capture_source 字段（必填）

每个 Source 文件的 frontmatter 必须包含 `capture_source` 字段，值只有两种：

```yaml
capture_source: external   # 来自 00_Capture/External（抖音、小红书等外部内容）
capture_source: personal   # 来自 00_Capture/Personal（个人随记、碎点子）
```

**这个字段是 Knowledge 分类的唯一依据。** Capture 区会定期清理，清理后只有这个字段能告诉 agent 这条 Source 应该进入 `Knowledge/External` 还是 `Knowledge/Personal`。不能依赖 Capture 文件是否还存在来判断。
